...
const FarmCards: React.FC = () => {
  ...
  const lfiIndex = farms.findIndex(
    ({ tokenSymbol }) => tokenSymbol === 'LFI',
  )

  const lfiPrice =
    lfiIndex >= 0 && stakedValue[lfiIndex]
      ? stakedValue[lfiIndex].tokenPriceInWeth
      : new BigNumber(0)

  const BLOCKS_PER_YEAR = new BigNumber(2372500)
  const LFI_PER_BLOCK = new BigNumber(60)
...
const FarmCard: React.FC<FarmCardProps> = ({ farm }) => {
  ...
  }, [lfi, lpTokenAddress, account, setHarvestable])

  const poolActive = true // startTime * 1000 - Date.now() <= 0

  return (
    <StyledCardWrapper>
      {farm.tokenSymbol === 'LFI' && <StyledCardAccent />}
      <Card>
        <CardContent>
          <StyledContent>
